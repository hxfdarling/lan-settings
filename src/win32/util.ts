export const DEFAULT_SETTINGS = [70, 0, 0, 0, 0, 0, 0, 0];
const SEP = [0, 0, 0];
const MIN_LENGTH = 4;
...
function parseBypass(result, bypass) {
  ...
  const index = bypass.indexOf('<local>');
  if (index !== -1) {
...
export const parseSettings = function(value) {
...
export const toRegBinary = function(settings) {
  if (!settings) {
